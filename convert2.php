<?php
/*
converts [amazon_textlink ...] shortcodes to [amazon link=""... /]
...
$sql = "SELECT * FROM `wp_postmeta` WHERE `meta_value` LIKE '%[amazon_textlink%'  ORDER BY `meta_id` DESC LIMIT 0,100";  //0-99, next is 100,100 for 100-199

$fp = fopen('yjsqlbackup2.csv', 'a+') or die($this->error_list);
...
function compute_replacement2($groups) {
	return '[amazon link="'.$groups[2].'" title="'.$groups[3].'" /]';
}
...
fwrite($fp,"meta_id;meta_value;meta_value_new\r\n") or die ("error writting!");
    ...
    while ($row = $result->fetch_row()) {
		...
		$regresult = preg_replace_callback("/(\[amazon_textlink asin=\')([A-Z0-9]{10})\'[^']text=\'([^']+)(\' [^]]+)\]/", 'compute_replacement2', $subject);
		//echo "#############<br/>".PHP_EOL;
